...
```
3. Map to references with Bowtie2, try both local and paired end using scripts. Calculate statistics. Run these commands wherever your merged fastq files are, be sure path to reference is correct and contains reference .bt2 index
```
...
```
4. Filter, sort, convert to bam file and index by running the following in the directory with your .sam files
```
  sbatch sam_to_sortedbam.slm
```
